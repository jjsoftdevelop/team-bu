...
router.post('/uploadFile', uploadHandler.single('uploadBox'), async (req, res, next) => {
    ...
    dbConnect.query("INSERT INTO file_log(filename, createdate, contentType, size, path, linkUrl, destination) VALUES(?,?,?,?,?,?,?)", [filename, createdate, contentType, size
        , path, linkUrl, destination
    ], (error, result) => {
        try {
            if (error) {
