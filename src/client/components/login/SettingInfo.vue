<template>
  <div>
    <div v-if="setPasswd">
      <div class="h3 text-info font-weight-bold mb-6">密碼設定</div>
      <form @submit.prevent="checkPasswd">
        <div class="mb-6">
          <label class="d-block text-info" for="passwd">您的密碼</label>
          ...
          <div class="mt-2 text-xs text-grey-300">
            <div :class="['mb-2', { 'text-success': passwd.length >= 6 }]">
              <b-icon
                icon="check2"
                v-if="passwd.length >= 6"
                class="mr-1"
              ></b-icon
              >需符合6個字元(含)以上
            </div>
            <div :class="{ 'text-success': checkNumAndStr }">
              <b-icon v-if="checkNumAndStr" icon="check2" class="mr-1"></b-icon
              >使用英數混合的字元組合
            </div>
          </div>
        </div>
        <div class="mb-6" style="height: 87px">
          <label class="d-block text-info" for="passwdck">再輸入一次密碼</label>
          <div class="position-relative">
            <b-form-input
              style="max-height: 62px"
              id="passwdck"
          ...
          <div v-if="!isSucessPasswd" class="mt-2 text-xs text-danger">
            兩次密碼不相同
          </div>
        </div>
        <div class="d-flex justify-content-end">
          <b-button
            :disabled="
              passwd.length < 6 || passwdCheck.length < 6 || !checkNumAndStr
            "
            :class="[
              'btn-m',
              {
    ...
    <form
      ...
      v-else
    >
      <label class="d-block text-info" for="nickname"
        >告訴我們該怎麼稱呼您</label
      >
      <b-form-input
        style="max-height: 62px"
      ...
      <div>
        <b-form-checkbox
          ...
          name="agreeTerm"
        >
          我同意所有 <a href="#" class="text-success">使用條款</a> 與
          <a href="#" class="text-success">隱私權政策</a>
        </b-form-checkbox>
        <div class="text-xs white-space-nowrap mt-2">
          <span class="text-info font-weight-bold">Team Bu</span>
          <span class="text-grey"
            >將會透過個人化電子郵件告知我產品與服務的相關訊息。</span
          >
        </div>
      </div>
      ...
      <div class="d-flex justify-content-end">
        <b-button
          :disabled="nickname.length === 0 || !isAgree"
          :class="[
            'btn-m',
            {
              'btn-disabled': nickname.length === 0 || !isAgree,
            },
          ]"
          type="submit"
          variant="success"
  ...
  methods: {
    checkPasswd() {
      if (this.passwd === this.passwdCheck) {
        ...
        this.isSucessPasswd = true;
        this.$emit("enterPasswd", this.passwd);
      } else {
        this.isSucessPasswd = false;
      }
    },
  },
  computed: {
